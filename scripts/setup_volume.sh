...
mkdir -p "$VOLUME_PATH"/{env,data,data/splits,project,outputs,cache,config,config/secrets}

# Create subdirectories for outputs
mkdir -p "$VOLUME_PATH"/outputs/{full_fine-tuning,frozen_backbone,hpo_results,zero_shot,optuna_studies,best_hyperparameters}

# Create cache subdirectories
mkdir -p "$VOLUME_PATH"/cache/huggingface/{transformers,datasets}

echo "Created directories:"
tree -L 2 "$VOLUME_PATH" 2>/dev/null || ls -la "$VOLUME_PATH"
...
HF_HOME=/mnt/volume/cache/huggingface
TRANSFORMERS_CACHE=/mnt/volume/cache/huggingface
...
echo "  4. Attach the volume at /mnt/volume"
echo "  5. Run: ${CYAN}bash /mnt/volume/project/scripts/instance_bootstrap.sh${NC}"
...
echo ""
echo -e "${CYAN}scp -P <PORT> root@<IP>:/mnt/volume/config/.rclone.conf ~/Desktop/rclone_backup.conf${NC}"
echo ""
